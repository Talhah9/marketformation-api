...
async function resolveCollectionId(collectionHandleOrId?: string | number): Promise<number | null> {
  ...
  const cc = await shopifyFetch(`/custom_collections.json?handle=${encodeURIComponent(handle)}&limit=1`);
  const ccId = cc.json?.custom_collections?.[0]?.id;
  if (cc.ok && ccId) return Number(ccId);

  // try smart collections
  const sc = await shopifyFetch(`/smart_collections.json?handle=${encodeURIComponent(handle)}&limit=1`);
...
export async function POST(req: Request) {
  try {
    ...
    const {
      ...
      collectionId,                // nouveau param optionnel (id direct)
    ...
    // ====== Vérif quota via Stripe (exemple pseudo, adapte à ta logique) ======
    // const customer = await stripe.customers.list({ email, limit: 1 });
    // const subStatus = ... // récup statut & plan
    // if (plan === 'starter') { vérifier le nombre de créations du mois (<=3) }
