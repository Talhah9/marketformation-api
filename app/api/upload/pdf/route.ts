// Upload PDF → Shopify Files CDN → { url }
...
async function shopify(path:string, init:RequestInit){
  const url = `https://${env('SHOPIFY_STORE_DOMAIN')}/admin/api/${process.env.SHOPIFY_API_VERSION||'2024-07'}${path}`;
...
export async function POST(req: Request){
  try{
    ...
    if(!(file instanceof File)) return new Response(JSON.stringify({error:"field 'pdf' manquant"}),{status:400,headers:{'Content-Type':'application/json',...CORS}});

    const attachment = Buffer.from(await file.arrayBuffer()).toString('base64');
    const out = await shopify('/files.json', { method:'POST', body: JSON.stringify({
      file: { attachment, filename: file.name || `course-${Date.now()}.pdf`, content_type: file.type || 'application/pdf' }
    })});
